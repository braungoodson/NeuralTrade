module.exports = function (PID,PORT,BUYIN,CASH,BTC,BUYCHIP,FUTURE,KEY,SECRET) {
	...
	function getFee() {
		tradeApi.getFee(getFeeHandler);
		function getFeeHandler(f) {
			f = f * .01;
			buyChip = chip + (chip * f);
			fee = f;
		}
	...
	function sellLive(r) {
		...
		//function getFeeHandler(f) {
			...
			//fee = f;
			var cost = btc * fee;
			//if ((sellChip <= btc) && (cost <= cash)) {
			if ((btc > 0.01) && cost <= cash) {
				console.log('Selling: ' + btc);
				tradeApi.sell(btc,r,callback);
				function callback(info) {
					if (info.success == 1) {
						cash = info['return'].funds.usd;
						btc = info['return'].funds.btc;
	...
	function callback(d) {
		...
		var o = {
			date: (new Date().getTime() + FUTURE) * .00000000001
		};
